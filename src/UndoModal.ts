...
export default class UndoModal extends Modal {
	...
	async onOpen() {
		...
		contentEl.createEl('div', {
			text: 'A single rollover command can be undone. Any text you may have added from those file(s) during that time may be deleted.'
		});
		contentEl.createEl('div', {
		...
		const undoHistoryInstance = this.plugin.undoHistory[0];
	...
	private async confirmUndo(undoHistoryInstance) {
